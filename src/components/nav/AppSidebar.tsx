...
const navItems = [
  ...
  { label: "Shared", icon: <SharedIcon className="h-5 w-5" />, disabled: true },
];
...
export function AppSidebar({
  ...
  return (
    ...
    >
      <nav className={`flex flex-1 flex-col ${isExpanded ? "px-3 py-2" : "items-center px-2 py-3"}`}>
              ...
              >
                ...
                {isExpanded && (
                  <>
                    ...
                    {item.chevron && <ChevronRightIcon className="h-3.5 w-3.5 text-[#9aa4b6]" />}
                  </>
                )}
              </button>
            );
        ...
        <div className="mt-0.5">
          ...
          >
            ...
            {isExpanded && (
              <span className="flex items-center gap-1 text-[#9aa4b6]">
      ...
      <div className={`border-t border-[#e4e7ec] ${isExpanded ? "px-4" : "px-2"} py-4 text-sm text-[#4c5667]`}>
        ...
        <div className={`mt-4 ${!isExpanded ? "flex justify-center" : ""}`}>
          <button
            onClick={onCreateClick}
            className={`flex items-center justify-center rounded-xl text-sm font-semibold shadow-sm transition ${
              isExpanded
                ? "w-full bg-[#1c64e4] px-4 py-2 text-white hover:bg-[#1550b6]"
                : "h-7 w-7 shrink-0 border border-[#9aa4b6] bg-white text-[#9aa4b6] hover:border-[#7a8499] hover:text-[#7a8499]"
            }`}
          >
            {isExpanded ? "+ Create" : "+"}
          </button>
  ...
  return (
    <button
      className={`mt-1 flex w-full items-center gap-2 rounded-lg ${
        isExpanded ? "px-2 py-2 text-left hover:bg-[#f4f6fb]" : "justify-center py-2"
      }`}
    >
      <span>{icon}</span>
